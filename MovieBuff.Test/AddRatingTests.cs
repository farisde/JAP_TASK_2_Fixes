...
namespace MovieBuff.Test
{
    ...
    class AddRatingTests
    {
        ...
        public async Task Setup()
        {
            ...
            #region InitializeRatingService
        ...
        public async Task AddNewRating_NewRatingInfo_SuccessfullyAddedRating()
        {
            ...
            var updatedRating = response.Data;

            Assert.Multiple(() =>
            {
                Assert.That(updatedRating.Value, Is.EqualTo(4));
            });
        }
        ...
        public async Task AddNewRating_NewRatingInfo_InvalidMovieID()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(response.Message, Is.EqualTo("Selected media ID doesn't exist."));
                Assert.That(response.Data, Is.Null);
            });
        }

        [Test]
        public async Task AddNewRating_NewRatingInfo_InvalidRatingValue()
        {
            var newRating = new AddRatingDto
            {
                RatedMediaId = 100,
